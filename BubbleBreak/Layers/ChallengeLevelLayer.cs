// -----------------------------------------------------------------------------------------------
//  <copyright file="ChallengeLevelLayer.cs" company="RetroTek Software Ltd">
//      Copyright (C) 2016 RetroTek Software Ltd. All rights reserved.
//  </copyright>
//  <author>Jared Mathes</author>
// -----------------------------------------------------------------------------------------------
// similar to LevelLayer but this class doesn't takea list of levels as a parameter. It just takes
// a single level. Figuring out which level should be played should be figured out outside of this
//------------------------------------------------------------------------------------------------
using System;
...
{
	public class ChallengeLevelLayer : CCLayerColor
	{
		...
		const float COIN_MULTIPLIER = 0.1f;		// multiplier to determin how many bonus coins the player gets - 10 points gets 1 coin
		...
		// Class constructor
		//---------------------------------------------------------------------------------------------------------
		public ChallengeLevelLayer (Level gameLevel, Player currentPlayer) : base (new CCColor4B (0,0,0))
		{
		...
		// CreateScene
		//---------------------------------------------------------------------------------------------------------
		public static CCScene CreateScene (CCWindow mainWindow, Level level, Player currentPlayer)
		{
			var scene = new CCScene (mainWindow);
			var layer = new ChallengeLevelLayer (level, currentPlayer);
		...
		void SetupUI ()
		{
			...
			if (activePlayer.IsMedKitActivated) AddChild (medKitSprite);
		...
		void StartScheduling()
		{
			Schedule (t => {
				...
				if (ShouldLevelEnd ()){
					...
					{
						didPassLevel = true;
						activePlayer.BranchProgression[activeLevel.BranchNum].LastLevelCompleted++;
						var coinsEarnedFromPoints = ConvertScoreToCoins (levelScore - levelPassScore);
						var coinsEarnedFromSequence = sequenceTotal * 3;
					...
					else
					{
						if (activePlayer.IsMedKitActivated) {
							...
							coinsEarned = (int)Math.Truncate ((double)sequenceTotal);
							activePlayer.Coins += coinsEarned;
							activePlayer.ConsumableAdd = availableAdditions;
							activePlayer.ConsumableSubtract = availableSubtractions;
							activePlayer.ConsumableCheckPoint = availableCheckpoints;
		...
		void EndLevel()
		{
			...
			var levelEndScene = ChallengeLevelFinishedLayer.CreateScene (Window, levelScore, activePlayer, didPassLevel, coinsEarned, activeLevel);
		...
		// ConvertScoreToCoins
		//---------------------------------------------------------------------------------------------------------
		// Takes the amout of overscore the player earned and converts to coins
		//---------------------------------------------------------------------------------------------------------
